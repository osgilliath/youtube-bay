## Youtube Bay - A YT video downloader
A youtube video downloader, from the terminal. <br/>
Download youtube videos, in any format you want.<br/>
Arising from my need to download videos from youtube, and the insane trust I have on third party websites, so here is my program, where you can download youtube videos in either video or audio format, your wish is this program's command.

## Insructions
Download and run the [yt-bay.exe](https://github.com/osgilliath/youtube-bay/blob/main/yt-bay.exe)

- Tell the program what format you want the audio in
- Paste the video url
- Boom, Done! Easy af!!

now if you want to use and check the [source code](https://github.com/osgilliath/youtube-bay/blob/main/yt-bay.py), it is included in the repo as well.

## For source code usage
To install the required libraries, run:
```bash
pip install imageio_ffmpeg
```

Then simply run the program:
```bash
python yt-bay.py
```

then repeat the same steps as the exe.


